...
class GoogleServiceAccount
{
    ...
    public static function credentials(): ?array
    {
        ...
        $projectId = trim((string) ($parsed['project_id'] ?? ''));
    ...
    public static function accessToken(string $scope): string
    {
        ...
        if ($creds === null) {
            throw new \RuntimeException('google_service_account_not_configured');
        }

        $cacheKey = sha1($scope);
        $now = time();
        $cached = self::$cachedTokens[$cacheKey] ?? null;
        ...
        if (!$ok) {
            throw new \RuntimeException('google_service_account_sign_failed');
        }
        ...
        if (!$response->successful()) {
            throw new \RuntimeException('google_service_account_oauth_failed');
        }
        ...
        if ($token === '') {
            throw new \RuntimeException('google_service_account_access_token_missing');
        }
