...
SOURCE_DIR="${SOURCE_DIR:-/var/www/html}"
BACKUP_DIR="${PROJECT_BACKUP_DIR:-/backups/project}"
INTERVAL_MIN="${PROJECT_BACKUP_INTERVAL_MIN:-1440}"
RETENTION_DAYS="${PROJECT_BACKUP_RETENTION_DAYS:-14}"
PROJECT_NAME="${PROJECT_NAME:-beautycrm_api}"
...
upload_to_s3() {
  ...
  if [ -z "$BACKUP_S3_BUCKET" ]; then
    echo "[project-backup] BACKUP_S3_ENABLED=true, but BACKUP_S3_BUCKET is empty. Skip S3 upload."
    return
  fi

  echo "[project-backup] uploading to s3://$BACKUP_S3_BUCKET/$key"
  local sse_args=()
  ...
  if ! aws_s3 s3 cp "$file" "s3://$BACKUP_S3_BUCKET/$key" --only-show-errors "${sse_args[@]}"; then
    echo "[project-backup] S3 upload failed for $file. Backup kept locally."
  fi
}
...
prune_s3() {
  ...
  if [ "$BACKUP_S3_ENABLED" != "true" ] || [ -z "$BACKUP_S3_BUCKET" ]; then
    echo "[project-backup] S3 prune skipped. enabled=$BACKUP_S3_ENABLED bucket_set=$([ -n "$BACKUP_S3_BUCKET" ] && echo yes || echo no)"
    return
  fi
  ...
  for key in $old_keys; do
    ...
    if ! delete_output="$(aws_s3 s3api delete-object --bucket "$BACKUP_S3_BUCKET" --key "$key" --only-show-errors 2>&1)"; then
      echo "[project-backup] failed to delete S3 object: $key"
      echo "[project-backup] delete error: $delete_output"
      continue
    fi
...
while true; do
  ...
  FILE="$BACKUP_DIR/${PROJECT_NAME}_${TS}.tar.gz"
  S3_KEY="${BACKUP_S3_PREFIX%/}/project/${PROJECT_NAME}_${TS}.tar.gz"

  echo "[project-backup] archiving project to $FILE"
  ...
  # S3 retention
  prune_s3 "${BACKUP_S3_PREFIX%/}/project"
